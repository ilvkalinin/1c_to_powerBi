...
report_dates AS MATERIALIZED (
...
), comparison_calendar AS MATERIALIZED (
  -- A monthly current-output batch still needs the preceding-year calendar
  -- to select the exact previous Monday / calendar date as its baseline.
  SELECT d::date AS report_date FROM params p,
       generate_series(p.comparison_start,p.horizon_end-1,interval '1 day') d
  WHERE extract(isodow FROM d)=1 OR extract(day FROM d)=1
  ...
  FROM report_dates d
  JOIN LATERAL (
    SELECT x.report_date FROM comparison_calendar x
  ...
  WHERE ab._fld672>=p.comparison_start-1 AND ab._fld671<p.horizon_end AND ab._fld672>ab._fld671
  ...
  CROSS JOIN params p WHERE d._fld4910rref=decode('859cb45b51f9e02c4fb16764c804af3d','hex')
    ...
    AND club._description::varchar<>'Детский развивающий центр' AND r._fld672>=p.comparison_start-1
  ...
  SELECT DISTINCT client_id FROM baseline_client_dates
), client_cards AS MATERIALIZED (
  SELECT u.client_id,cl._fld1507::date birth_date,
  ...
  FROM used_clients u JOIN public._reference141x1 cl ON cl._idrref=u.client_id
), tenure_history AS MATERIALIZED (
  -- One effective-history scan replaces an index probe for every retained
  -- baseline client at every report date.
  SELECT h._fld5655rref client_id,h._period,
    CASE h._fld5656rref WHEN decode('bc06e4b21430ebfb44a67a65c46d41f9','hex') THEN 'New' WHEN decode('9e369ac955bf602149e17b549b0f1498','hex') THEN 'Ex' WHEN decode('91e4594e35ce15d847c4a3f32e1e18f2','hex') THEN 'Renew' ELSE 'Не указано' END::text membership_tenure,
    lead(h._period) OVER (PARTITION BY h._fld5655rref ORDER BY h._period,h._fld5656rref) next_period
  FROM public._inforg5654 h JOIN used_clients u ON u.client_id=h._fld5655rref
), client_static AS MATERIALIZED (
  SELECT u.report_date,u.client_id,c.birth_date,c.gender,coalesce(t.membership_tenure,'Не указано') membership_tenure
  FROM baseline_client_dates u JOIN client_cards c ON c.client_id=u.client_id
  LEFT JOIN tenure_history t ON t.client_id=u.client_id AND t._period<u.report_date::timestamp
    AND (t.next_period IS NULL OR t.next_period>=u.report_date::timestamp)
), classified AS (
  ...
  SELECT *,
    ...
    CASE WHEN current_is_child THEN 'Дети' WHEN birth_date IS NULL OR birth_date=date '0001-01-01' THEN 'Не указано' WHEN extract(year FROM age(report_date,birth_date))<14 THEN 'Дети' WHEN extract(year FROM age(report_date,birth_date))<18 THEN 'Юниоры' ELSE 'Взрослые' END::text current_age_group
  FROM classified
)
SELECT scope_level,report_date,comparison_type,comparison_date,encode(baseline_club_ref,'hex')::text baseline_club_id,
  current_age_years,current_age_group,
...
GROUP BY scope_level,report_date,comparison_type,comparison_date,baseline_club_ref,current_age_years,current_age_group,gender,membership_tenure;
