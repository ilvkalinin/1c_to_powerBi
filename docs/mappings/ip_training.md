...
До группировки всё равно нужен временный технический ключ исходной тренировки. `InfoRg7006` содержит `Period`, `Recorder`, `LineNo`, но текущий SQL не выбирает `LineNo`, а join с услугами может размножать строки. Считать `(Period, Recorder, LineNo)` ключом тренировки до проверки нельзя; на VM-2 этот технический ключ хранить не требуется.
...
| `training_count` | количество квалифицированных тренировок в целевой комбинации | `COUNT` логических тренировок после устранения технического размножения | `integer`/`bigint` | нет | CONFIRMED business / technical key pending | сверка с текущим `COUNT(Контрагент)` |
...
3. Правила `Active`, `Posted` и `Marked`.
4. Фактические типы, объёмы и контрольные значения.
