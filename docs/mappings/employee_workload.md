...
## Компонент C: раздельное присутствие СКУД — physical implementation deferred

Персональный целевой grain — `employee × presence_date × actual club`; для
неоднозначного домена — отдельный `presence_date × club × attribution_status`
без `employee_id`. Текущий M группирует по display-name, однако будущие
...
| `employee_id` | сотрудник | `Reference225.ID` через `Reference225.Fld2504RRef = AccumRg7575.Fld7576RRef` | только exact-one domain; 29,431 no-link и 708 multi-link visits уходят в separate non-personal product | `text` | нет | CONFIRMED target decision | EPD-V04, BR-043 |
...
`MULTIPLE_EMPLOYEES`, а его minutes используют ту же подтверждённую формулу.
...
| `attribution_status` | `NO_EMPLOYEE` при отсутствии client-link; `MULTIPLE_EMPLOYEES` при `count(employee)>1` | `text` / нет | CONFIRMED user decision, BR-043 / EPD-V04 |
...
| `Document325`, `AccumRg7575`, `Reference225`, `Reference59` | СКУД и сотрудник-клиент/ИП | CONFIRMED source; BR-043 separates 29,431 no-link and 708 multi-link visits without employee | [EPD decision](../reports/employee_presence_day_attribution_decision_2026-08-28.md) |
...
| CONFIRMED user decision | СКУД сотрудника | Exact current-M domain: 29,431 qualified visits without employee and 708 with two employees; BR-043 stores them separately without employee. Historical EW-V05 broader visit domain had 1,292 multi-link visits. | EPD-V04 / BR-043 |
