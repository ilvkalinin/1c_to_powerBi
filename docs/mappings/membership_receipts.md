...
| `source_stage` | исходный NEW/RENEW/EX | `Reference59` | `Fld694` | current GUID mapping | `text` | да | контракт | CONFIRMED current DAX | `стаж_контракта` | GUID coverage |
...
| `payment_type` | тип оплаты | `Reference59` + current M | `Fld699`, contract type | `9bd3ea4748457ee94b2011de6d9687d7` → `Рекарринг`; другой non-NULL → `Предоплата`; `NULL` → `Услуга`; `Кредит`/`Бесплатный` отображаются в `Предоплате`, когда не исключены scope | `text` | нет | движение | CONFIRMED common rule | current M + BR-024 | coverage |
| `payment_source` | канал продажи | recorder documents + product name | несколько | current COALESCE/override; сохранять `Клуб`, `Website`, `App`, `App сотрудника`, `Web customer`, `Рассрочка` | `text` | да | движение | CONFIRMED target categories | M/DAX + user decision 2026-07-31 | coverage |
...
| `product_age_category` | продуктовая возрастная категория | `Reference163` | `Fld1741` + name/type | `Взрослые`/`Дети`/`Юниоры`; current `Детские секции` → `Дети` | `text` | да | контракт | CONFIRMED target rule | PBIT M/DAX + user correction 2026-07-31 | coverage |
