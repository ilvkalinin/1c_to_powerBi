...
| `payment_type` | тип оплаты | `Reference59` + current M | `Fld699`, contract type | `Кредит`/`Бесплатный` → `Предоплата`; остальные → `Предоплата`/`Рекарринг`/`Услуга` | `text` | нет | движение | CONFIRMED target rule | M + user decision 2026-07-31 | coverage |
