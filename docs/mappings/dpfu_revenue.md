...
Mapping основан на текущих SQL/M/DAX, бизнес-описании и metadata. Это логическое сопоставление, а не решение о числе физических таблиц; применённая S3-RR-EXEC-001 миграция фиксируется отдельно в ADR-0005.
...
При S3-RR-EXEC-001 текущие ДПФУ-строки получили
скрытое техническое значение `revenue_scope = dpfu`. Оно не передаётся в
модель ДПФУ; `mart.v_dpfu_ancillary_revenue` возвращает только этот scope.
Источник Power BI не переключался и меняется только отдельным согласованным
пакетом.
