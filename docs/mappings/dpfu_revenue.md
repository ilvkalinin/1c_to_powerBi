...
| `club_id` | клуб выручки ИП | `AccumRg7370.Fld7372` | `bytea` source → protected/text key | DECISION_REQUIRED | S3-IP-REVENUE-001: 92 049/178 022 qualified BR-003 movements не имеют movement-club, но имеют club договора; current M сохраняет пустой клуб через `LEFT JOIN` | user decision on `NULL` preservation vs contract-club attribution |
