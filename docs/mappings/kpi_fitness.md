...
SQL-факт: `CONFIRMED — решение пользователя 2026-07-30`. Для вовлечённости
используется общий `mart.client_base_daily`; совместимость grain, `club/network`
scope и периода подтверждена ADR-0031 и S3-CBD-ADMISSION-001. Пустая схема
создана по S3-CBD-DDL-001; initial load остаётся отдельным пакетом.
