...
| `club_id` | устойчивый клуб | ссылки клубов регистров; Excel через `СпрЦЗ` | целевой технический ключ планируется как канонический текстовый hex из физической ссылки; текущий PBIT использует отображаемое название клуба | `text` | допускается только для текущей ветки ИП: 17 дневных ключей на её полном диапазоне (SV-049) | то же | ASSUMPTION design; текущая модель подтверждена по названию клуба | M + metadata | source type, orphan, unique club dimension |
...
| CONFIRMED | ключи/статусы/знаки внутренней PostgreSQL-части | движение, знаки и итоговый ключ проверены SV-035–SV-048 | текущая логика PBIT сохранена; Excel не входит в проверку |
