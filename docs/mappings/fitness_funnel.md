...
равна при разных атрибутах. Для них пользователь выбрал максимальный срок
`Reference59.Fld693`; отсутствие равенств этого второго ранга ещё должно быть
проверено. До этой проверки target не создаётся;
evidence:
...
максимальной дате приобретения `Reference59.Fld674`, затем по максимальному
сроку `Reference59.Fld693`. Равенство обоих рангов не даёт права выбрать
произвольный договор: остаётся `VALIDATION_PENDING`.
