...
Основание — переданные описание, Power Query, DAX и PBIT
`Pbit_old/Отчет по обращениям.pbit` (SHA-256
`9c46cdc6847ec69617a1abf8eaa5cd0635777109d58ea162558391991bb1242f`), утверждённый пользователем 2026-08-18 как база первого релиза этой страницы. SQL и объект витрины не
...
CR-V08 подтвердил current PBIT-знаменатель посещений на июле 2026: 307 911
непустых ссылок на контракт и итоговое значение 307 911 после группировки по
дню, клиенту, основному клубу доступа и клубу посещения. Три дополнительных
строки относительно ключа `день × клиент × клуб посещения` образуются из-за
разного основного клуба доступа; они не дедуплицируются без нового правила.

PBIT подтверждает отдельный scope страницы «Отработка обратной связи»: все
interactions `Reference67` type «Обратная связь», созданные с 2025-01-01 до
текущей даты, без `Jivo` в имени interaction. Фиксированного фильтра тем,
