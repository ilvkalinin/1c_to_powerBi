...
Статус: `BUSINESS MAPPING COMPLETE / ARCHITECTURE DESIGNED — ADR-0015 / TECHNICAL VALIDATION PARTIALLY VALIDATED — SV-072`.
...
|---|---|---|---|
| `mart.prebooking_state_event` | одно событие состояния одной записи | `(state_period, recorder_tref, recorder_rref, line_no)` технически подтверждён | своевременность, нетто-записи, отмены и детализация клиента |
| `mart.dpfu_plan_assignment` | дата плана × клуб × подразделение × тренер × плановый клиент | `InfoRg6612` технически уникален; текущий логический key подтверждён PC-V10 | REUSE общего детального плана |
| `mart.group_lesson` | одно групповое занятие | `Document279.ID` технически подтверждён | вместимость, активные записи и пришедшие |

Техническая уникальность подтверждена SV-072; она не отменяет риски
business-semantic дедупликации legacy join через `Document329.VT4352`.
...
| Сравнение ключей | Технические ключи state-event и плана, а также ID урока уникальны; legacy ПЗ → VT4352 остаётся one-to-many. | PARTIALLY VALIDATED — SV-072 |
...
|---|---|---|---|
| PARTIALLY VALIDATED | `InfoRg7006` | Технический ключ уникален и `Active=false=0`; 104 строки не имеют Enum448. Бизнес-смысл кодов остаётся current M/DAX. | SV-072 PC-V02, PC-V07; сохранить current inner join. |
| VALIDATION_FAILED | `InfoRg7006 → Document279/329` | Полиморфная ссылка не имеет orphan/пересечений, но service расходится с документом у 1 871 ГП / 22 093 ПЗ, club — у 4 ПЗ. | SV-072 PC-V03–PC-V05; не подменять регистровое измерение документным. |
| VALIDATION_FAILED | `Document329.VT4352` | Current M соединяет её без номера строки: 59 581 добавочная legacy-строка. | SV-072 PC-V09; сохранять результат BR-018, дедупликация требует отдельного решения. |
| VALIDATED | состояния документов | Unposted/marked строки измерены в обеих ветках; новый фильтр не подтверждён и не добавляется. | SV-072 PC-V04. |
| PARTIALLY VALIDATED | PII клиента | Непустые коды уникальны, но есть один пустой; связка отмена → посещение и техническое применение permissions/RLS остаются следующей проверкой. | SV-072 PC-V06; BR-017. |
| VALIDATED | план и групповые итоги | Технические/логические ключи плана, ID урока, capacity и однострочный `InfoRg8675` подтверждены; два one-to-many набора агрегируются отдельно. | SV-072 PC-V10–PC-V12. |
