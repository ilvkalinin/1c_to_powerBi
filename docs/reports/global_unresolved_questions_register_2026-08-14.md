...
| 21 | Отчёт по обращениям | `BLOCKED` | Physical feedback/HTML/follow-up ordering подтверждены bounded controls. Шесть тем и четыре воронки имеют exact match, но документированный фильтр `Продажа клип-карт Рецепция` отсутствует в `Reference89`. Есть только `ASSUMPTION`-кандидат `Продажа клип карты Рецепция`; без исходного current SQL/M/DAX среди материалов проекта он не заменяет scope. Также остаются знаменатель посещений, Power BI reconciliation и rerun/SLA. | `mart.crm_interaction`, `mart.v_feedback_interaction`; SV-088/098; ADR-0016 |
