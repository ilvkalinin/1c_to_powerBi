...
| 30 | Членство для правления | `VALIDATION_REQUIRED` | States, non-additive KPI controls и source SLA остаются к проверке; recurring unit наследует подтверждённое `contract × payment_period` (SV-096). Сверка с фактическими итогами Power BI не является pre-creation blocker. | `mart.membership_receipt_movement`, `mart.membership_contract_kpi_unit`; SV-083/096; ADR-0017 |
