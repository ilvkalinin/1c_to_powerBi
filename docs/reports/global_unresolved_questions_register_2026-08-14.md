...
| 21 | Отчёт по обращениям | `VALIDATION_REQUIRED` | Physical feedback/HTML/follow-up ordering и базовый funnel scope подтверждены: BR-023 связывает документированное имя с единственным `Reference89._idrref`. Остаются speed/quality/Jivo/state scope, business-grain знаменателя посещений, Power BI reconciliation и rerun/SLA. | `mart.crm_interaction`, `mart.v_feedback_interaction`; SV-088/098; ADR-0016 |
