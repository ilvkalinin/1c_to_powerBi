...
`DataModelSchema`.  The current model has active relationships from
...
activity.  Therefore a coupon-row tie is not merely a hidden technical choice:
it can change date-sliced result values.
...
The coupon branch is different.  Current M performs `Table.Distinct` on
`client_code, club, division_name, employee, service_name, class_start` while
retaining other columns, including `visit_date`; 149 such groups contain
different retained payloads.  The M query specifies no stable tie order.  A
physical table must choose whether to preserve all rows (changes coupon counts
and hours) or select one deterministic row (changes at least date-sliced
legacy detail).  Neither choice is authorized by BR-018.

## Stage-3 boundary

No Stage 3 SQL plan exists and no physical object may be created.  Before a
reviewed DDL/DML plan can be prepared, the user must choose the deterministic
coupon-row policy for the 149 ambiguous current-M keys.  The first release
otherwise preserves the current PZ/GZ state filters, VT multiplicity and raw
coupon-duty subtraction, including observed negative clean duties; replacing
that subtraction by interval union would be a separate methodology change.
