...
- `Pbit_old/Отчет по обращениям.pbit`, SHA-256 `9c46cdc6847ec69617a1abf8eaa5cd0635777109d58ea162558391991bb1242f` — архивный PBIT-артефакт; не заменяет согласованные current M/DAX.
...
5. Старый dataset задаёт шесть тем, а набор скорости/качества намеренно иной:
   пользователь подтвердил не объединять эти фильтры. Архивный PBIT показывает
   полный feedback-source без фиксированного списка тем, воронок или кампаний;
   это сохранённый potential artifact, но не current scope. Базовые шесть тем
   и пять воронок зафиксированы; CR-V05G подтвердил current Jivo-exclusion за
   все закрытые месяцы 2026-01—07 (8 904 из 55 633 feedback rows), а также
   три непустых физических статуса в каждом месяце. Открытым остаётся scope
   speed/quality по current материалам.
