...
5. Старый dataset задаёт шесть тем, а набор скорости/качества намеренно иной:
   ...
   пять воронок зафиксированы; CR-V05E подтвердил current Jivo-exclusion на
   bounded июле (919 из 6 326 feedback rows), но speed/quality и status GUID
   scopes остаются открытыми. CR-V05F подтвердил три непустых физических
   статуса без их привязки к пользовательским именам.
