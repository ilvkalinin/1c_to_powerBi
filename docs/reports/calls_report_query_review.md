...
- `Pbit_old/Отчет по обращениям.pbit`, SHA-256 `9c46cdc6847ec69617a1abf8eaa5cd0635777109d58ea162558391991bb1242f` — согласованная пользователем 2026-08-18 базовая M/DAX-логика первого релиза для страницы «Отработка обратной связи».
...
5. Старый dataset задаёт шесть тем, а скорость/качество использует отдельную
   таблицу `ОС со звонками`. PBIT подтверждает, что её постоянный source scope
   — все `Reference67` с type «Обратная связь», created 2025-01-01..today и
   исключением имени `%Jivo%`; фиксированного списка тем, воронок или кампаний
   в странице нет. Сохранённые выборы месяца/воронки/кампании — состояние
   слайсеров, не правило отчёта. CR-V05G подтвердил Jivo-exclusion за все
   закрытые месяцы 2026-01—07 (8 904 из 55 633 feedback rows), а также три
   непустых физических статуса в каждом месяце.
...
| V-05 | точный набор type/status/theme/funnel/campaign GUID и Jivo-исключений для набора шести тем; отдельно подтвердить physical scope speed/quality | CR-V05A: шесть тем и четыре воронки имеют ровно один physical match; BR-023 фиксирует пятую воронку через единственный `Reference89._idrref`. PBIT подтвердил, что speed/quality не имеет отдельного фиксированного topic/funnel/campaign filter; Jivo и три статуса подтверждены. |
