...
5. Старый dataset задаёт шесть тем, а набор скорости/качества намеренно иной:
   ...
   воронок, тем и Jivo-исключений всё ещё требуют V-05. Базовые шесть тем и
   пять воронок зафиксированы; CR-V05G подтвердил current Jivo-exclusion за
   все закрытые месяцы 2026-01—07 (8 904 из 55 633 feedback rows), а также
   три непустых физических статуса в каждом месяце. Их точное соответствие
   «Выполнено» / «Не выполнено» / «Отменено» закреплено current SQL; открытым
   остаётся только scope speed/quality.
