...
Контрольный июль 2026 воспроизвёл PBIT-знаменатель посещений: 307 911
исходных строк и столько же непустых ссылок на контракт дали 304 114 строк
после current группировки и итог 307 911. Это `COUNT(contract_ref)`, не
число разных клиентов. Для 304 111 ключей `день × клиент × клуб посещения`
выявлены три дополнительные строки из-за разного `ОсновногоКлубаДоступа`.
Текущая группировка сохраняется; новая дедупликация не вводится. Сверка с
фактическим числом Power BI не выполняется, поскольку контрольное число не
предоставлено.
