...
подтверждает точные таблицы и DAX накопленного трафика. В gymdb выполнен только
...
- `Накопленный трафик факт2` — точное `UNION('Задания 2024','Задания 2025')`;
  основной `Задания` включает 2026. Это подтверждённая текущая реализация и
...
| `CONFIRMED current PBIT / VALIDATION_PENDING physical` | Точная мера накопленного трафика использует `UNION(Задания 2024, Задания 2025)`, двухмесячное окно, отмены и активированные контракты. | На контрольном месяце MF-V08 обязан подтвердить полный набор task, отмен, client-count contracts и итог меры. |
| `CONFIRMED current PBIT` | Задания для накопленного трафика ограничены 2024–2025. | После 2026 требуется ручная корректировка; historical result сохраняется по BR-018. |
