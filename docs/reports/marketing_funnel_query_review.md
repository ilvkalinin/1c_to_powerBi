...
Рассмотрены текущие SQL/M/DAX и бизнес-описание. Архивный PBIT
`Pbit_old/Воронка.pbit` (SHA-256
`ddf6115ed9f8f8c017c0ca0cb0ce61c61f1694aaf5bbfe6665147280b8e2d0c3`)
сохраняет potential artifact и не заменяет current rules. В gymdb выполнен только
...
- Архивный PBIT содержит `UNION('Задания 2024','Задания 2025')` и меру с
  двухмесячным окном, отменами и активированными контрактами. Это potential
  artifact, не current DAX; риск отсутствия накопленного трафика после смены
  года и exact current алгоритм остаются открытыми.
...
| `VALIDATION_PENDING` | Exact current алгоритм накопленного трафика не зафиксирован среди согласованных SQL/M/DAX. Архивный PBIT с union 2024/2025, two-month cohort, отменами и контрактами сохранён как potential artifact. | Нельзя применить или проверить этот вариант без решения о его отношении к current report. |
| `VALIDATION_PENDING` | Current охват лет для накопленного трафика не подтверждён. | Не переносить из архивного PBIT ограничение 2024–2025 без отдельного решения. |
...
| MF-V08 | алгоритм накопленного трафика | current DAX/M сначала подтверждён; затем на согласованном месяце совпадают задания, контракты, отмены и направление |
