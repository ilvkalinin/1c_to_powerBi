...
|---|---|---|
| `VALIDATION_FAILED` | `InfoRg6798` содержит более одного подходящего контракта на задание: SV-080 — 100 строк bridge, 36 заданий, 21 multi-contract task; 85 контрактов в multi-contract task groups, максимум 16 контрактов на задание. | Нельзя суммировать строки bridge в task-level `contract_count` или молча дедуплицировать контракт. Нужна отдельная decision до Stage 3. |
