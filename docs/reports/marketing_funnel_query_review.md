...
утверждён пользователем 2026-08-18 как базовая DAX-логика первого релиза для накопленного трафика. В gymdb выполнен только
...
| `CONFIRMED user-approved PBIT / VALIDATION_PENDING physical` | Точная мера накопленного трафика использует `UNION(Задания 2024, Задания 2025)`, двухмесячное окно, отмены и активированные контракты. | На контрольном месяце MF-V08 обязан подтвердить полный набор task, отмен, client-count contracts и итог меры. |
