...
Статус: `COMPLETE / NO DATABASE EXECUTION`.
...
Read-only доказательства уже сохранены в `SV-035`–`SV-050`: технические
ключи, знаки и состояния внутренних регистров, anti-overlap ДПФУ/рецепции,
ветви членства, уникальность дневного ключа и связи PBIT. В текущем
`STAGE_1_LOCAL_ANALYSIS` новые подключения и выполнение SQL не производились.
...
Выполнение отложено: VPN к VM-1 не доступен. До запуска разрешены только
read-only запросы; `:schema` и `:table` заменяются результатом preflight
`pg_catalog`.
