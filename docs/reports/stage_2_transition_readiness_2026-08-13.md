...
1. `PROGRAM ENTRY` — разрешить начать Stage 3 только с отчётных продуктов
   класса `A` ниже; SQL-план опасных операций рассматривается при одобрении
   каждого полного product-admission пакета.
...
Перед одобрением каждого полного product-admission пакета пользователю
показывается его SQL-план. После одобрения все включённые операции выполняются
без повторных согласований. Подробная граница и safeguards зафиксированы в
граница и safeguards зафиксированы в
...
| 30 | Членство для правления | PARTIALLY VALIDATED SV-083 | C | class-C review: переиспользовать BR-015/016, recurring-платёж как KPI-единицу и раздельные роли дат; SQL ждёт physical key/state и board reconciliation/non-additive controls |
...
- review SQL-плана до одобрения полного пакета, отдельные миграции и rollback;
