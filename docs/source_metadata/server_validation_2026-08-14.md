...
Статус: `PARTIALLY VALIDATED / PBIT DISTINCT OUTCOME PENDING`. Выполнен
2026-08-18 на live `gymdb` в `BEGIN READ ONLY` под `gymdb_readonly`,
`statement_timeout = 30s`. Точный SQL сохранён как MR-V09A в
[membership_receipts_2026-08-11.sql](validation_sql/membership_receipts_2026-08-11.sql).
...
шага `Table.Distinct`. Нельзя ни вычесть эти строки, ни агрегировать их по
новому правилу: сначала остаётся проверить фактический результат текущего
удаления полных дубликатов, затем сохранить риск или доказательство.
